...
1.  [Процесите в детайл](#процесите-в-детайл)
    1. [Процес](#процес)
    1. [Създаване на процес](#създаване-на-процес)
    1. [Контролен блок]()
    1. [Processing State Machine - Състояния на процеси](#processing-state-machine-–-състояния-на-процеси)
...
1.  [Работа с байтове и комуникация](#работа-с-байтове-и-комуникация)
...
- Управление на паметта (Memory Management)
  ...
  **_RAM паметта_** представлява **_огромен масив от байтове_**, който е **енергозависим**. Операционната система се грижи _всички процеси да стигнат до RAM паметта_ и оттам до _процесора_. **Резултатите** на процесора също се записват в RAM паметта преди да отидат на хард диска. Ядрото има следните **_3 функции_**, когато работи с оперативната памет:

  1. да _заделя_ и _освобождава_ оперативната памет, когато е нужно
  2. да следи кои части от паметта на кои процеси са (_на кого принадлежат_)
  3. да решава кои _данни и процеси трябва да са в RAM_ паметта

- Управление на входно-изходните устройства (I/O Management)

  Много е важно да се отбележи, че комуникацията с всяко входно-изходно устройство е **система с буфери и кеширане**.

  **_Драйверите_** определят как операционната система да запише / прочете и разбере байтовете от I/O устройствата. Затова всяко устройство има драйвер – програма, която знае как да работи с даден модел устройство.

  Ядрото предоставя единен **_интерфейс_** (конвенция, набор от правила, начин на работа), който е част от самото ядро, за работа с типове устройства. Устройства **не знаят за съществуването** на този интерфейс.

  **_Драйверите_** са посредника между устройствата и интерфейса. Работа с I/O устройства се изпълнява с инструкции, които **_не_** могат да бъдат изпълнени в непривилегирован режим. Само ОС може да работи с хардуерната система директно.

  `Ядро <-> интерфейс <-> драйвер буфер + устройство`

  ОС управа буфера на това устройство. Не взима тази информация моментално и затова ядрото управлява тези буфери. Входно-изходната система също така кешира тази информация – в кеш, рам, регистър, тъй като буферите на входно-изходните устройства са много бавни.

  Но тази система има друга важна задача – **_spooling_**. **_Spooling_** е концепция, методология, идея, при която се **разделя работата** на входно-изходното устройство **на малки части** заради хардуерни лимитации. Проблемът с I/O устройствата е, че _буферът е с изключително малък_ и ограничен размер. **_Устройствата само четат буфера_**. И когато не е достатъчен I/O системата на ядрото се грижи за това да _запълва постоянно_ буфера, като _разделяме работата_ на малки части. Това съществува, поради хардуерните _лимитации_ на малките и бавни входно-изходни устройства.

  _Прекрасен пример за това биват принтерът и голямият материал, който искаме да принтеме. Принтерът може да има буфер от 10MB, докато ние искаме да принтиме 100MB. Тогава ядрото се грижи да раздели това на 10 части и да ги принтира по една част на всяка итерация._

- Управление на външно-запаметяващите устройства (Storage Management)

  Устройства за съхранение на данни се използват, тъй като искаме данните да оцелеят при спиране на системата, както тези, които не се побират за RAM паметта.

  Типове устройства за съхранение:

  - **WORM** - Записваме веднъж четете много, наистина много. WORM описва устройство за съхранение на данни, в което информацията, веднъж написана, не може да бъде модифицирана. Тази защита срещу запис гарантира, че данните не могат да бъдат подправени, след като бъдат записани на устройството
  - **RW** - писане и четане
  - **Архивиране** - специални устройства - много бавни, но reliable. За долгосрочно съхранение на данни

  Хардуерните устройства се различават по скорост, обем и начин на работа. Целта на ядрото е да скрие тези различия. Затова операционната система създава се концепцията за **_файл_**, за да ни е по-лесно.
  ...
  Ядрото реализира две абстракции: файл и файлова система, които позволяват да се управляват външни запаметяващи устройства. ОС може да създава, трие, мести, променя съдържание, да променя правата за достъп.
...
1. Get pid – Опрерационната памет заделя за всеки процес има уникален индентификатор `pid` във времето. Като този индентификатор е уникално число, което може да се преизползва, след като процесът с даденото `pid` умре. `pid === ProcessID` и е позитивно число от 0 до 2^16.

   **_НЕ МОЖЕ ДА ИМА 2 ЕДНИ И СЪЩИ PID_**
...
За какво се използва state? Процесите преминават през много етапи по време на техния „мизерен“ (Даниел Александров) живот. Ние ще изучаваме 5. Oперационната система имплементира функционалността за състояноя на процесите - Processing State Machine:
...
Процесите от време на време стигат и до състояние, когато нямат полза (нужда) от процесорно време. Примерно да използваме i/o устройство, което е бавно в своята същност. Процесът трябва да чака. Затова процесът отива в блокиран режим, докато не се случи нещото, което се чака. И след това се връща обратно.
...
- `int open(char *filename, int flags);`
  ...
  Те представляват макроси. Няколко флага може да се комбинират с `|` (побитово или), тъй като това са конкретни битове

- `int close(int fd);` - деалокира файлов дескриптор

  **_Критична грешка е да НЕ СЕ ЗАТВОРИ файловия дескриптор_**

- `int read(int fd, void *buffer, size_t count);` - записва _count_ на брой байтове от fd в _buffer_. При успех връща броя на байтовете, които е прочел.

- `int write(int fd, void *buffer, size_t count);` - сваля _count_ на брой байтове от RAM паметта в Hard диска (при read е обратното)
...
- `off_t lseek(int fd, off_t offset, int whens);`
  **_off_t е integer_**
  ...
  - offset - колко байта да се премести курсора
  - whens - (от)къде да се премести курсора

  При whens може да се използват следните макроси:

  - **_SEEK_SET_** -> премести курсора на толкова байта от началото на файла
  - **_SEEK_CUR_** -> текущата позиция + толкова байта напред
  - **_SEEK_END_** -> премести курсора на толкова байта след края на файла

  **_lseek_** премества накъде сочи курсора, асоцииран с файл дескриптора. Връща новата позиция на курсора при успех.

  Местенето на курсора **не променя файла**. Това е само указател където да се чете/пише следващия байт.
...
Файловият дескриптор е абстракция, чрез която можем да работим с файлове, това е неотрицателноо целочислено число - номер (от гледна точка на програмата). От гледна точка на ОС, файловият дескриптор е структура. Всеки файлов дескриптор се асоциира с курсор и само с един файл. Файловите дескриптори са динамично алокиран ресурс.

В Unix операционните системи, първите три файлови дескриптора по подразбиране са:

- STDIN (стандартен вход) -> 0
- STDOUT (стандартен изход) -> 1
- STDERR (стандартна грешка) -> 2.
...
Endianness е _реда на байтовете в числото_. В зависимост от това, какъв е реда на байтовете, числото се чете по различен начин. Има два вида endianness:

- **little endian** - младшите байтове са на по-малко значещо място
- **big endian** - старшите байтове са на по-малко значещо място

Примерно при мрежовите карти се използва big endian (тъй като наподобява на stream), а при процесорите се използва little endian (тъй като е по-ефикасен).

**_Важно е да се отбележи, че от endianness не зависи какъв е реда на битовете в числото, той винаги е един и същ._**

### <stdint.h>

- `uint8_t` - unsigned int от 8 бита

- `uint16_t` - unsigned int от 16 бита

- `uint32_t` - unsigned int от 32 бита

- `uint64_t` - unsigned int от 64 бита

### Bob & Alice

- Bob и Alice са двама хора, които се комуникират чрез някакъв канал (например, сокет)

### Видове комуникация

- shared memory - комуникация чрез обща, споделена памет
- pipes -
- sockets - комуникация само че през интернет
- signals -
- files - комуникация чрез четене и писане във файлове
